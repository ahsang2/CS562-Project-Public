...
def runPGD(real_image_dir, cloaked_output_dir):
    subprocess.run('python3 pgd_attack.py --real_image_dir=' + real_image_dir + ' --cloaked_output_dir=' + cloaked_output_dir, shell=True)
    return
...
def avgs():
    ...
    cloaked_metrics = defaultdict(list)
    uncloaked_metrics = defaultdict(list)

    # Loop through each experiment's CSV file in the folder
    for experiment_file in os.listdir('experiments') :  # You need to get a list of experiment files
        #files = ['clip_metrics_pgd_p0.csv', 'img_metrics_cloaked_p0.csv']
        files = ['clip_metrics_pgd_p0.csv']
        for file in files:
            ef = 'experiments/' + experiment_file + '/' + file

            with open(ef, 'r') as file:
                reader = csv.reader(file)
                for row in reader:
                    metric_name, metric_value = row
                    # Append metric value to the list corresponding to its metric_name
                    cloaked_metrics[metric_name].append(float(metric_value))  # Convert to appropriate type if needed

    for experiment_file in os.listdir('experiments') :  # You need to get a list of experiment files
        #files = ['clip_metrics_uncloaked_p0.csv', 'img_metrics_uncloaked_p0.csv']
        files = ['clip_metrics_uncloaked_p0.csv']
        for file in files:
            ef = 'experiments/' + experiment_file + '/' + file
            ...
            with open(ef, 'r') as file:
                ...
                for row in reader:
                    metric_name, metric_value = row
                    # Append metric value to the list corresponding to its metric_name
                    uncloaked_metrics[metric_name].append(float(metric_value))  # Convert to appropriate type if needed

    # Calculate average metric values
    average_metrics = {}
    for metric_name, metric_values in cloaked_metrics.items():
        average_metrics[metric_name] = sum(metric_values) / len(metric_values)

    average2_metrics = {}
    for metric_name, metric_values in uncloaked_metrics.items():
        average2_metrics[metric_name] = sum(metric_values) / len(metric_values)

    # Now `average_metrics` contains the average values of each metric across all experiments
    print("cloaked", average_metrics)
    print("uncloaked", average2_metrics)
    
def run_experiments():
    for celeb_directory in os.listdir("CelebA"):
        experiment_directory = "experiments/experiment_" + celeb_directory
        ...
        clear_memory()
        runPGD("CelebA/" + celeb_directory, experiment_directory + "/input-base")
